...
class SportsRequestBeneficiary(Base):
    ...
    sports_request_id = Column(
        Integer,
        ForeignKey("sports_requests.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    sports_category_id = Column(Integer, ForeignKey("sports_categories.id"), nullable=False)
...
class SportsRequestBeneficiarySupportType(Base):
    __tablename__ = "sports_request_beneficiary_support_types"

    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    beneficiary_id = Column(
        Integer,
        ForeignKey("sports_request_beneficiaries.id", ondelete="CASCADE"),
        nullable=False,
    )
    support_type_id = Column(Integer, ForeignKey("sports_support_type.id"), nullable=False)
